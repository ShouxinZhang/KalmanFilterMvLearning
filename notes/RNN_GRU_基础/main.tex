...
\begin{tikzpicture}[rnndiagram, x=1cm, y=1cm]
  ...
  \node[rnnblock, minimum width=3.6cm, minimum height=1.3cm] (cell) at (4.2,0) {\shortstack{RNN cell\\$\phi(W_x x_t + W_h h_{t-1} + b)$}};
  ...
  \draw[rnnskip] (h.north) -- node[right] {next state} (hnext.south);
\end{tikzpicture}
\caption{A plain RNN combines the current input with the previous hidden state to produce a new state, then optionally reads out a prediction.}
\label{fig:rnn-en}
\end{figure}
...
\begin{tikzpicture}[rnndiagram, x=1cm, y=1cm]
  ...
  \node[rnnblock, minimum width=2.9cm, minimum height=1.1cm] (cand) at (6.1,0) {\shortstack{candidate\\$\tilde{h}_t$}};
  \node[rnnblock, minimum width=2.2cm, minimum height=1.1cm] (mix) at (9.0,0) {blend};
  ...
  \draw[rnnarrow] (cand.east) -- node[above] {write} (mix.west);
  \draw[rnnarrow] (z.east) |- (mix.north);
  \draw[rnnskip] (hprev.east) .. controls +(1.8,0) and +(-1.8,-0.8) .. node[below] {keep} (mix.south);
  \draw[rnnarrow] (mix.east) -- (h.west);
  \draw[rnnarrow] (h.south) -- (y.north);
\end{tikzpicture}
\caption{A GRU keeps the same recurrent backbone, but gates the flow of information so the old state is blended with a candidate update.}
\label{fig:gru-en}
\end{figure}
